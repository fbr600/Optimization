/*
	...
	This is the code which calculates one time the GA with the parameters at the
	top, and give us the best individual after the maxiter iterations.
*/
...
int 				n 		= 10000; 	//population (required to be an even number)
unsigned long int 	seed 	= 1;
int 				t 		= 100;	//number of elements to do the parents selec
int 				posCr 	= 17;	//numb of positions choosed in crossover
int 				maxiter = 100000;	//number max of iterations
double				mutq	= 0.7;
...
int main ()
{
	int **a,i,j,**p,*best;
	unsigned int d;
	FILE *f1,*fres,*f2,*fsol;

	fres 	= fopen("tspend.txt","w");	
	f1 		= fopen("dist.txt","r");
	f2 		= fopen("cities.txt","r");
	fsol	= fopen("bestway.txt","w");	
	
	srand(time(NULL));
	
	//Allocating a matrix for cities distances
	...
	//write the list of the cities for the best solution
	Writesol(best,f2,fsol);

	free(a);free(p);free(best);free(f1);
}
void Writesol(int *a,FILE *fcit,FILE *fsol)
{
	char c;
...
void ga(int **pop, int *best, int **a)
{
	...
	do
	{
		...
		for(i=0;i<n;i++) 
		{
			...
			if(fit>f[i])
			{
				if(f[i]<fit2)	{fit2 = f[i]; /*printf (" %d ",iter);*/for(j=0;j<m;j++) best1[j] = pop[i][j];}
				k = i;
				fit = f[i];
			}
		}
		printf("iter %d dist is %d total min %d\n",iter,fit,fit2);
